(ns clj_hypergraph_db.hdm_parser.hdm_uml_model_manager_neo4j
...
(defn get-target-class-of-role-neo4j
  [association-name role-name]
  (-> @model :associations-neo4j association-name :roles role-name :target-class))

(defn get-roles-index
  [association-name role-name]
  (->> role-name (.indexOf (-> @model :associations association-name :roles-order)) inc))

(defn get-roles-index-neo4j
  [association-name role-name]
  (->> role-name (.indexOf (-> @model :associations-neo4j association-name :roles-order)) inc))
...
(defn get-class-and-all-subclasses-list-neo4j
  [class-name]
  (loop [result []
         subclasses [class-name]]
    (if (not-empty subclasses)
      (recur (concat result subclasses) (apply concat (map #(-> @model :classes-neo4j % :extended-by) subclasses)))
      result)))
...
(defn get-class-and-all-superclasses-list-neo4j
  [class-name]
  (loop [result []
         superclasses [class-name]]
    (if (not-empty superclasses)
      (recur (concat result superclasses) (apply concat (map #(-> @model :classes-neo4j % :extends) superclasses)))
      result)))
...
(defn get-pk-list-neo4j
  [class-name]
  (for [class-name (get-class-and-all-superclasses-list class-name)
        pk-name (-> @model :classes-neo4j class-name :pk-set)]
    pk-name))
...
;TODO LINK
(defn add-class-instance-return-with-link-neo4j
  [class-name]
  (let [{class-handle :handle
         instance-counter :instance-counter} (-> @model :classes-neo4j class-name)
        instance-handle (neo4j-add-node {:class class-name})
        instance-link (-> @instance-counter str keyword (hg-add-link [class-handle instance-handle]))]
    (swap! instance-counter inc)
    [instance-handle instance-link]))
...
(defn add-class-instance-return-with-link-neo4j
  [class-name]
  (first (add-class-instance-return-with-link-neo4j class-name)))
...
;TODO LINK
(defn add-attribute-instance-neo4j
  [class-instance-handle attribute-name attribute-data]
  (hg-add-link attribute-name [class-instance-handle (neo4j-add-node {:attribute attribute-data})]))
...
;TODO LINK
(defn add-association-instance-neo4j
  [association-name]
  (let [{instance-counter :instance-counter
         roles-order :roles-order
         association-handle :handle} (-> @model :associations-neo4j association-name)
        instance-value (-> @instance-counter str keyword)]
    (swap! instance-counter inc)
    (hg-add-link instance-value (repeat (inc (count roles-order)) association-handle))))
